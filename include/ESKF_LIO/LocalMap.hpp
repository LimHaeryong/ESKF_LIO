...
namespace ESKF_LIO
{
    ...
    public:
        ...
        LocalMap(double voxelSize, size_t maxNumPointsPerVoxel, size_t numVoxelOffsets)
            : voxelSize_(voxelSize), maxNumPointsPerVoxel_(maxNumPointsPerVoxel), voxelOffsets_(initVoxelOffsets(numVoxelOffsets)) {}
        ...
        struct Voxel
        {
            ...
            std::optional<std::pair<Eigen::Vector3d, double>> nearestSearchInVoxel(const Eigen::Vector3d &point, double maxDistSq) const;
        };

        void updateLocalMap(const PointVector &points);
        std::optional<std::pair<Eigen::Vector3d, double>> nearestSearch(const Eigen::Vector3d &point, double maxDistSq) const;
    ...
    private:
        ...
        std::vector<Eigen::Vector3i> initVoxelOffsets(int numVoxels);
