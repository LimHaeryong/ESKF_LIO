...
void Odometry::run()
{
  double cloudPreprocessorElapsedTime = 0.0, filterUpdateElapsedTime = 0.0, mapUpdateElapsedTime = 0.0;
  int numFrames = 0;
  while (!exitFlag_) {
    if (visualize_) {
  ...
  std::cout << "cloud preprocessor average elapsed time = " << std::fixed << cloudPreprocessorElapsedTime / numFrames * 1000 << " ms\n";
  std::cout << "filter update average elapsed time = " << std::fixed << filterUpdateElapsedTime / numFrames * 1000 << " ms\n";
  std::cout << "map update average elapsed time = " << std::fixed << mapUpdateElapsedTime / numFrames * 1000 << " ms\n";
}
}  // namespace ESKF_LIO
