...
Eigen::Isometry3d ICP::align(const PointCloud& cloud, const LocalMap& localMap, const Eigen::Isometry3d& guess)
{
  ...
  for (int i = 0; i < maxIteration_; ++i)
  {
    ...
    auto correspondence =
        localMap.correspondenceMatching(tmpCloud.points_, maxCorrespondenceDistSquared_, matchingRmse_);
...
Eigen::Isometry3d ICP::computeTransform(Correspondence& correspondence) const
{
  ...
  Eigen::Map<Eigen::Matrix<double, 3, Eigen::Dynamic>> srcMatrix(srcPoints[0].data(), 3, srcPoints.size());
  Eigen::Map<Eigen::Matrix<double, 3, Eigen::Dynamic>> dstMatrix(dstPoints[0].data(), 3, dstPoints.size());
